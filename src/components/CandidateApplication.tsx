...
const CandidateApplication: React.FC<CandidateApplicationProps> = ({ user, onSuccess, onCancel }) => {
    ...
    const handleAnalyze = async () => {
        if (!formData.manifesto || !formData.experience) return;
        setIsAnalyzing(true);
        ...
        // Sync background with experience for backend compatibility
        const finalFormData = {
            ...formData,
            background: formData.experience // Map experience to background
        };
        ...
        const profile = await evaluateCandidate(
            user.username,
            formData.manifesto,
            finalFormData.background,
            formData.reasonForContesting,
            formData.experience,
                    ...
                    <div className="space-y-6 animate-in slide-in-from-right-4">
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input
                                    type="text"
                                    placeholder="e.g. The Technocrats"
                            ...
                            <div>
                                <label className="block text-xs font-mono text-gray-500 uppercase mb-2">Contact Info</label>
                                <input
                                    type="text"
                        ...
                        <div>
                            <label className="block text-xs font-mono text-gray-500 uppercase mb-2">Reason for Contesting</label>
                            <input
                                type="text"
                                placeholder="What drives you to lead?"
                                value={formData.reasonForContesting}
                                onChange={e => setFormData({ ...formData, reasonForContesting: e.target.value })}
                                className="w-full bg-surface-50 border border-surface-300 rounded-lg p-3 text-white focus:border-neon-blue outline-none"
                            />
                        </div>

                        <div>
                            <label className="block text-xs font-mono text-gray-500 uppercase mb-2">Manifesto / Core Agenda</label>
                            <textarea
                                placeholder="Detailed platform and vision..."
                        ...
                        <div>
                            <label className="block text-xs font-mono text-gray-500 uppercase mb-2">Experience & Background</label>
                            <textarea
                        ...
                        {/* Hidden field for backward compatibility if needed, or mapped to experience */}
                        <input type="hidden" value={formData.background} />

                        <div className="pt-4 flex justify-between items-center">
                            <button onClick={onCancel} className="text-gray-500 hover:text-white text-sm">Cancel</button>
                            <button
                                onClick={handleAnalyze}
                                disabled={!formData.manifesto || !formData.experience}
