...
import { Poll, PollOption, PollComment, ConsciousnessLayer } from '../../types';
import { Share2, MessageCircle, AlertTriangle, Send, ThumbsUp, ThumbsDown, CornerDownRight, MessageSquare, Bookmark, Wand2, Activity, Check, X, Edit2, Trash2 } from 'lucide-react';
...
const PollCard: React.FC<PollCardProps> = ({
  ...
  const OwnerControls = () => {
    ...
    return (
      <div className="absolute top-0 right-0 z-50 p-4 flex gap-2">
        <button
          onClick={(e) => { e.stopPropagation(); onEdit && onEdit(poll.id); }}
          className="bg-surface-200 text-white px-3 py-1 rounded-lg font-bold text-xs flex items-center gap-1 hover:bg-neon-blue hover:text-black transition-colors border border-surface-300"
        >
          <Edit2 size={12} /> Edit
        </button>
        <button
          onClick={(e) => { e.stopPropagation(); onDelete && onDelete(poll.id); }}
          className="bg-red-500/10 text-red-500 px-3 py-1 rounded-lg font-bold text-xs flex items-center gap-1 hover:bg-red-500 hover:text-white transition-colors border border-red-500/20"
        >
    ...
    >
      ...
      <div className="flex justify-between items-start mb-4 relative z-10">
        <div className="flex items-center gap-2">
          ...
          {poll.isEdited && (
            <span className="px-2 py-1 bg-surface-200/50 text-[10px] font-bold text-gray-400 uppercase tracking-wider rounded border border-surface-300/50 flex items-center gap-1">
              <Edit2 size={8} /> Edited
            </span>
          )}
          {poll.authorId && (typeof poll.authorId === 'object') && (
            <div className="flex items-center gap-2 ml-2">
              <img
                src={(poll.authorId as any).avatarUrl || 'https://api.dicebear.com/7.x/avataaars/svg?seed=anon'}
                alt="Author"
                className="w-5 h-5 rounded-full border border-surface-300"
              />
              <span className="text-xs text-gray-400 font-medium">
                {(poll.authorId as any).username || 'Anonymous'}
              </span>
            </div>
          )}
        </div>
